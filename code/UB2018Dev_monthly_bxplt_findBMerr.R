...
scens <- list(
  ...
  "Aug 2018 + Fix" = "Aug2018_2019_9000,DNF,2007Dems,IG_9000,Most"
...
minvals <- cbind.data.frame(
  ...
  scen_res %>%
    dplyr::filter(Variable == "BlueMesa.Outflow") %>%
  ...
  #check min BM outflow from offc
  scen_res %>%
    dplyr::filter(Variable == "MorrowPoint.Outflow") %>%
    dplyr::filter(Scenario == names(scens[1]))  %>% 
    dplyr::group_by(MonthNum) %>%
    summarise(min = min(Value)),
...
#plot the below 
## plot 
pdf(paste0(ofigs,'/UBresDevPlots_Monthly_BMFix.pdf'), width=9, height=6)
# for(i in 1:1){
